...
postRouter.get("/mypostdetail",(req,res)=>{
 ...
 pool.query(sql,[id],(err,result)=>{
   ...
   if(err)throw err;
   //查询本笔记赞的数量
   var sql1="SELECT count(1) dzs FROM zl_storgr WHERE sg_pid=? and sg_flag='0'";
   pool.query(sql1,[id],(err,res1)=>{
		if(err)throw err;
	   //查询本笔记收藏的数量
	   var sql2="SELECT count(1) scs FROM zl_storgr WHERE sg_pid=? and sg_flag='1'";
